...
#ifndef GATE_GATESIPM_HH
#define GATE_GATESIPM_HH
...
/* structure for SiPM pulses info*/
...
class GateSiPM : public GateVPulseProcessor
{
public:

  //! Constructs a new GateSiPM attached to a GateDigitizer
  GateSiPM(GatePulseProcessorChain* itsChain,  const G4String& itsName);
...
public:
  ...
  GatePulseList *ProcessPulseList(const GatePulseList *inputPulseList) override;

  void DescribeMyself(size_t indent) override;

private:

  GateSiPMMessenger *m_messenger;
...
public:
  ...
  G4int wichMicroCell( G4ThreeVector localPos) const;
  ...
  void setCellsDimentions(GateVVolume* volume);
 ...
 /* Set the pitch of microcells */
  ...
  void setDurationPulse(G4double endSignal);
...
private:
  G4double m_current_time_value, m_current_pulse_value, m_current_pulse_amplitude;
  G4int m_trackID;
  G4int m_runID;
  G4int m_stepID;
  G4int m_parentID;
...
//  G4long m_seed;
  ...
  void build_nested_volumeID(size_t level_current, GateOutputVolumeID outputVol);
  void FindLevelsParams();
  G4int compute_generalDetId_from_volumeID(GateVolumeID *volumeID);
